...
export function extractProfileFromMessages(messages: ChatMessage[]): {
  ...
  // Allow company names to start with any letter case
  const companyPatterns = [
    /(?:from|at|with|work for|work at|representing)\s+([A-Za-z][A-Za-z0-9\s&]+?)(?:\.|,|$|\s+and\s|\s+My\s|\s+my\s)/i,
    /(?:company|business|organization)\s+(?:is\s+)?([A-Za-z][A-Za-z0-9\s&]+?)(?:\.|,|$)/i,
  ]
